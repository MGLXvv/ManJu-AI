...
|---|---|---|---|---|---|
| Auth | `http-partial` | 账号密码登录已有 HTTP Adapter；统一拦截器可注入 Token | 明文记住密码；Refresh Token、Cookie、退出和会话恢复未冻结；正式入口仍导出 Mock 常量 | 删除明文密码；建立 Session Repository；把存储键和错误枚举移出 Mock；增加会话恢复测试 | 更新 `auth.http.ts`、DTO、错误映射和 Session Adapter |
| Project | `http-reserved` | 列表、详情、新建、更新、删除、导入、导出入口存在 | 路径曾发生漂移；导入导出数据语义未冻结；真实分页未验证 | 统一路径常量；冻结前端 Project Contract；补路径和 Mapper 测试 | 调整 `project.http.ts`、DTO、分页 Mapper |
...
| Generation Task | `blocked` | 任务类型、状态、Mock 任务和结果 Guard 已存在 | HTTP `create/update` 不可用；业务生成端点与通用任务端点并行；缺少恢复和有限并发 | 建立 GenerationTaskGateway 和状态机；先迁移 Mock；定义轻量结果和 Workspace 刷新 | 实现任务创建、查询、取消、重试、轮询或订阅 Adapter |
...
| Setting Asset | `http-partial` | 设定资产列表、创建、更新、候选图和生成入口已预留 | `/settings` 与 `/aidrama/assets` 两类路径语义可能重叠；上传仍可能传 Data URL；结果要求完整 asset | 建立能力状态、上传抽象和轻量结果；明确资产 Workspace 所有权 | 更新 `setting.http.ts` 或统一到 Asset Workspace Adapter |
...
| Voice | `http-reserved` | `/voices` CRUD Adapter 已存在 | 字段、分页、上传音频和试听 URL 生命周期未验证 | 建立 Voice DTO Fixture 和上传能力状态 | 更新 `voice.http.ts`、DTO、Mapper 和上传 Adapter |
| Resource Library | `readonly` | 列表读取已有 HTTP Adapter | 创建、编辑、删除当前会直接返回 unsupported；HTTP 层仍引用 Mock 文件夹数据 | 建立能力注册表；移除 HTTP 对 Mock 的依赖；定义文件夹来源 | 补资源库 CRUD 和项目复用接口 |
...
| System | `http-reserved` | 样式、权限、消息等模块入口存在 | 当前优先级低；权限模型和系统配置所有权未确定 | 保持能力状态为未验证，不影响主流程 | 接入系统 DTO、权限和消息接口 |
...
| Export | `http-partial` | JSON 草稿和配音结果导出可用 | 项目导出、媒体打包和剪映工程语义未冻结 | 区分前端 JSON 导出与后端归档导出；能力状态明确 | 接入异步导出任务和文件下载 |
| Registration / Code Login / Third-party Login | `deferred` | Mock 逻辑和页面结构部分保留 | HTTP Adapter 当前明确 unsupported；不属于当前主验收 | 页面隐藏或标记暂缓；避免误触 | 产品和后端确认后单独立项 |
